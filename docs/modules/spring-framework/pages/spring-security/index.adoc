...
image:https://img.shields.io/badge/Spring%20Documentation-2088E9?logo=quickLook&logoColor[link="https://docs.spring.io/spring-security/reference/index.html",window=_blank]
image:https://img.shields.io/badge/Spring%20Boot%20Documentation-2088E9?logo=quickLook&logoColor[link="https://docs.spring.io/spring-boot/reference/web/spring-security.html",window=_blank]
...
You'll have to invoke {spring-boot-api-spec-url}/org/springframework/boot/web/client/RestTemplateBuilder.html#basicAuthentication(java.lang.String,java.lang.String)[`🟢 RestTemplateBuilder#basicAuthentication(String username,
String password)`^] when configuring Bean xref:spring-framework:servlet-stack/spring-web-client.adoc#_externalize_base_uri[here].
...
* xref:spring-framework:spring-security/spring-authorization-server.adoc[*Authorization Server*] - verifies the identity of the user then issues access tokens to the application
* xref:spring-framework:spring-security/spring-resource-server.adoc[*Resource Server*] - the resource to access
...
Next, you need to add `@Bean` for {spring-security-api-spec-url}/org/springframework/security/oauth2/client/OAuth2AuthorizedClientManager.html[`⚪ OAuth2AuthorizedClientManager`^] which will be *responsible for the overall management* of {spring-security-api-spec-url}/org/springframework/security/oauth2/client/OAuth2AuthorizedClient.html[`🟢 OAuth2AuthorizedClient`^]. The primary responsibilities include:

. *Authorizing (or re-authorizing) an OAuth 2.0 Client* by leveraging an {spring-security-api-spec-url}/org/springframework/security/oauth2/client/OAuth2AuthorizedClientProvider.html[`⚪ OAuth2AuthorizedClientProvider`^]
. *Delegating the persistence* of an `🟢 OAuth2AuthorizedClient`, typically using an {spring-security-api-spec-url}/org/springframework/security/oauth2/client/OAuth2AuthorizedClientService.html[`⚪ OAuth2AuthorizedClientService`^] or {spring-security-api-spec-url}/org/springframework/security/oauth2/client/web/OAuth2AuthorizedClientRepository.html[`⚪ OAuth2AuthorizedClientRepository`^]
...
----
<1> {spring-security-api-spec-url}/org/springframework/security/oauth2/client/registration/ClientRegistrationRepository.html[`⚪ ClientRegistrationRepository`^] checks if client `registrationId` provided in authorization request is present in registration repository (enriched from registration properties)
<2> `⚪ OAuth2AuthorizedClientProvider` is *a strategy* for authorizing (or re-authorizing) an OAuth 2.0 Client. Implementations will typically implement a specific *authorization grant type*. Here we build {spring-security-api-spec-url}/org/springframework/security/oauth2/client/ClientCredentialsOAuth2AuthorizedClientProvider.html[`🟢 ClientCredentialsOAuth2AuthorizedClientProvider`^] which is an implementation for `client_credentials` grant.
<3> {spring-security-api-spec-url}/org/springframework/security/oauth2/client/AuthorizedClientServiceOAuth2AuthorizedClientManager.html[`🔴 AuthorizedClientServiceOAuth2AuthorizedClientManager`^] is an implementation of `⚪ OAuth2AuthorizedClientManager` which persist `🟢 OAuth2AuthorizedClient`:
...
You'll also need to implement `@Component` for {spring-api-spec-url}/org/springframework/http/client/ClientHttpRequestInterceptor.html[`⚪ ClientHttpRequestInterceptor`^] which allows to *modify the outgoing request and/or the incoming response*:
...
<2> `⚪ ClientRegistrationRepository` returns {spring-security-api-spec-url}/org/springframework/security/oauth2/client/registration/ClientRegistration.html[`🔴 ClientRegistration`^] *enriched* with our `springauth` registration properties.
<3> {spring-security-api-spec-url}/org/springframework/security/oauth2/client/OAuth2AuthorizeRequest.html[`🔴 OAuth2AuthorizeRequest`^] represents *a request* the `⚪ OAuth2AuthorizedClientManager` *uses to authorize (or re-authorize) the client* identified by the provided client `registrationId` (`springauth`) and principal (`oidc-client`).
...
If your test is annotated with `@RestClientTest`, then you need to *mock* `⚪ OAuth2AuthorizedClientManager` with {spring-boot-api-spec-url}/org/springframework/boot/test/mock/mockito/MockBean.html[`@MockBean`^]:
...
and add a {spring-boot-api-spec-url}/org/springframework/boot/test/context/TestConfiguration.html[`@TestConfiguration`^]
...
----
<1> This {spring-security-api-spec-url}/org/springframework/security/oauth2/core/OAuth2AccessToken.html[`🟢 OAuth2AccessToken`^] will be returned together with `🟢 OAuth2AuthorizedClient` after invoking {spring-security-api-spec-url}/org/springframework/security/oauth2/client/OAuth2AuthorizedClientManager.html#authorize(org.springframework.security.oauth2.client.OAuth2AuthorizeRequest)[`OAuth2AuthorizedClientManager#authorize(OAuth2AuthorizeRequest authorizeRequest)`^]
