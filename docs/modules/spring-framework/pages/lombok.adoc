...
image:https://img.shields.io/badge/Documentation-2088E9?logo=quickLook&logoColor[link="https://projectlombok.org/features/",window=_blank]
...
{lombok-api-spec-url}/lombok/Getter[`@Getter`^]::
Put on any field to make lombok build a standard getter.

{lombok-api-spec-url}/lombok/Setter[`@Setter`^]::
Put on any field to make lombok build a standard setter.

{lombok-api-spec-url}/lombok/NonNull[`@NonNull`]::
...
{lombok-api-spec-url}/lombok/NoArgsConstructor[`@NoArgsConstructor`^]::
...
{lombok-api-spec-url}/lombok/RequiredArgsConstructor[`@RequiredArgsConstructor`^]::
...
{lombok-api-spec-url}/lombok/AllArgsConstructor[`@AllArgsConstructor`^]::
Generates an all-args constructor. An all-args constructor requires one argument for every field in the class.

{lombok-api-spec-url}/lombok/ToString[`@ToString`^]::
...
{lombok-api-spec-url}/lombok/EqualsAndHashCode[`@EqualsAndHashCode`^]::
...
{lombok-api-spec-url}/lombok/Synchronized[`@Synchronized`^]::
...
{lombok-api-spec-url}/lombok/Locked[`@Locked`^]::
Like `@Synchronized`, but using {java-api-spec-url}/java.base/java/util/concurrent/locks/ReentrantLock.html[`🟢 ReentrantLock`^]. It is *recommended for Virtual Threads* (introduced in Java 20)
